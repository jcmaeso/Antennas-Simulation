% SUB-ARRAY EXAMPLE2 (extestsub2)
% 
%
% Description :
%
% A circular sub-array of 5 patches is defined and then used as the array element.
% For verification the array of sub-arrays is only 3x2 elements, this of
% course can be changed as required, see file (line 135)
%
% Detail :
% 
...
%
% Figure 1  shows the geometry for the sub-array detail (5 patches)
...
circ_array(5,1,(0.7*lambda),(0.7*lambda),'patchr',0,'yes'); % Define the sub-array
...
plot_geom3d1(1,0,21);  % Plot sub-array 3D geometry on default figure(21)
...
rect_array(2,3,lambda*1.5,lambda*1.5,'interp',0);      % Array definition using the sub-array element ('interp')
...
ax=axis;                                               % zoom out by 1.5
axis(ax*1.5);


[SUBtheta0b,SUBpat0b]=calc_theta(-90,0.5,90,0,'tot','yes');    % Phi=0 pattern using 'interp'
