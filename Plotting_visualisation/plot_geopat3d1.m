function plot_geopat3D1(deltheta,delphi,polarisation,normalise,surftype,gpratio,fignum)
% Plot 3D pattern and array geometry in selected figure, suggest using 15 or greater, 
% numbers 1 to 14 already assigned as defaults for other plots.
%
...
% surftype.....Surface plot type (string) 
% gpratio......Geometry to pattern ratio  (float) 
...
%               'ar'    - Axial Ratio
%               'tau'   - Tilt of polariasation ellipse
...
%
% Options for surftype are :
%
%               'surf' -  Surface plot, interpolated shading
%               'mesh' -  Wire mesh plot
%
...
%
%
% e.g. plot_geopat3D1(10,15,'tot','no','surf',4,15)
%
%         z
...
switch surftype
  case 'surf',% OK
  case 'mesh',% OK
  otherwise,fprintf('\nUnknown surface type, options are : "surf" or "mesh"\n');...
            fprintf('Surface type set to "surf"\n');surftype='surf';
end
...
figure(fignum);
clf;
chartname=sprintf('3D %s GeoPat',upper(polarisation));
set(fignum,'name',chartname);
hold on;

if strcmp(surftype,'mesh')
 mesh(x,y,z,PlotData);
 view([60,20]);
 colormap('jet');
 hidden off;
...
sub_geopat3D(gpratio);  % Plot the geometry
