function plot_wave_slice(xrng,xsteps,yrng,ysteps,xrot,yrot,zrot,xoff,yoff,zoff,polarisation,fignum)
% Plot E-field parameters visualised as a 3D wave surface added to the figure 
% specified by fignum. Note that this function is for visualisation only.
...
%
%
% Usage: plot_wave_slice(xrng,xsteps,yrng,ysteps,xrot,yrot,zrot,xoff,yoff,zoff,polarisation,fignum)
%
...
% Add the wave slice to the selected figure
figure(fignum);
hold on;
caxis([-lambda/2,lambda/2]);
surf(XG,YG,ZG,PlotData);  % Plot data in global coordinates, adding to 3D geometry if already plotted
colormap('jet');
